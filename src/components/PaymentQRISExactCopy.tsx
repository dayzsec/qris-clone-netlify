// === COPIED from previous canvas version ===
// If you already have the component there, paste the entire component here.
// For convenience, a compact version is included below (identical behavior):
...
for (let r=0;r<rows;r++){for(let c=0;c<cols;c++){if(r<q||c<q||r>=rows-q||c>=cols-q)cells[r][c]=0;}}
function F(x:number,y:number){const s=7;for(let r=0;r<s;r++){for(let c=0;c<s;c++){const rr=y+r,cc=x+c;if(rr<0||cc<0||rr>=rows||cc>=cols)continue;const b=r===0||c===0||r===s-1||c===s-1;const cn=r>=2&&r<=4&&c>=2&&c<=4;cells[rr][cc]=b||cn?1:0;}}}
F(q,q);F(cols-q-7,q);F(q,rows-q-7);
const svgSize = cols*cell; const rects: JSX.Element[] = [];
for (let r=0;r<rows;r++){for(let c=0;c<cols;c++){if(!cells[r][c])continue;rects.push(<rect key={`${r}-${c}`} x={c*cell} y={r*cell} width={cell} height={cell} />)}}
return (<svg width={svgSize} height={svgSize} viewBox={`0 0 ${svgSize} ${svgSize}`} xmlns="http://www.w3.org/2000/svg" className="bg-white"><rect x="0" y="0" width={svgSize} height={svgSize} fill="white"/><g fill="black">{rects}</g></svg>);
}
